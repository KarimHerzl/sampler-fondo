...
def classify(f):
    ...
    #   bianca/sterrato  L ~0.82   |   asfalto  L ~0.66   |   verde alto = vegetazione
    # Il tono caldo (WARM) NON discrimina qui: si decide sulla luminosita' (L).
    if f["ExG"] > 0.15:   return "coperto"     # traccia inerbita / ombra verde
    if f["L"] >= 0.78:    return "sterrato"    # chiaro = battuto/bianca
    if f["L"] <= 0.70:    return "asfalto"     # scuro = asfalto
    return "incerto"                           # zona grigia: serviranno piu' campioni
